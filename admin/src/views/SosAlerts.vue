...
import { AlertTriangle, BadgeCheck, CheckCircle2, History, MapPinned, Radio, ShieldAlert, XCircle } from '@lucide/vue'
...
<template>
  <div class="space-y-5">
    ...
    >
      <div class="w-full max-w-3xl rounded-lg bg-white p-6 shadow-2xl">
        <div class="flex items-start justify-between gap-4 border-b border-slate-100 pb-3">
          <div>
            <h3 class="text-lg font-black text-slate-950">Lịch sử ca SOS đã hoàn thành</h3>
            <p class="mt-1 text-xs text-slate-500">Danh sách các ca cấp cứu khẩn cấp đã xong. Chọn một ca để xem lại bản đồ chặng đi hoặc tiếp tục cập nhật hành trình giọt máu.</p>
          </div>
          <button class="rounded-md border border-slate-200 px-3 py-2 text-xs font-black text-slate-500 hover:bg-slate-50" @click="showCompletedModal = false">
            Đóng
          </button>
        </div>
        ...
        <div class="mt-4 max-h-[60vh] overflow-y-auto pr-1">
          <div v-if="completedAlerts.length" class="grid gap-3">
            <article
              ...
              :key="alert.id"
              class="flex flex-col justify-between gap-3 rounded-lg border border-slate-200 p-4 transition hover:bg-slate-50 sm:flex-row sm:items-center"
            >
